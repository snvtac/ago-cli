...
test("collectClaudeSessionsForDir skips slash-command boilerplate and surfaces the command name", async () => {
  await withTempDir(async (dir) => {
    const slash = [
      JSON.stringify({ type: "user", sessionId: "slash", timestamp: "2026-03-01T00:00:00.000Z", message: { role: "user", content: "<local-command-caveat>Caveat: The messages below were generated by the user while running local commands. DO NOT respond.</local-command-caveat>" } }),
      JSON.stringify({ type: "user", sessionId: "slash", timestamp: "2026-03-01T00:00:01.000Z", message: { role: "user", content: "<command-name>/model</command-name>             <command-message>model</command-message>" } }),
      JSON.stringify({ type: "user", sessionId: "slash", timestamp: "2026-03-01T00:00:02.000Z", message: { role: "user", content: "<local-command-stdout>Set model to Sonnet</local-command-stdout>" } }),
    ].join("\n");
    await fs.writeFile(path.join(dir, "slash.jsonl"), `${slash}\n`, "utf8");

    const sessions = await collectClaudeSessionsForDir(dir);
    assert.equal(sessions.length, 1);
    assert.equal(sessions[0]?.preview, "/model");
  });
});

test("collectClaudeSessionsForDir skips whitespace-only content parts and uses the next real part", async () => {
  await withTempDir(async (dir) => {
    const line = JSON.stringify({
      type: "user",
      sessionId: "ws",
      timestamp: "2026-03-02T00:00:00.000Z",
      message: { role: "user", content: [{ type: "text", text: "   " }, { type: "text", text: "the real prompt" }] },
    });
    await fs.writeFile(path.join(dir, "ws.jsonl"), `${line}\n`, "utf8");

    const sessions = await collectClaudeSessionsForDir(dir);
    assert.equal(sessions[0]?.preview, "the real prompt");
  });
});
